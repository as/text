...
func New(sp, pad image.Point, b *image.RGBA) *Scroll {
	return &Scroll{win.New(sp, pad, b), 0}
}

const msgSize = 64 * 1024

func (w *Scroll) BackNL(p int64, n int) int64 {
	R := w.Bytes()
	if n == 0 && p > 0 && R[p-1] != '\n' {
		n = 1
	}
...
func (w *Scroll) FrameScroll(dl int) {
	if dl == 0 {
		return
	}
	org := w.org
